...
import scalation.calculus.Calculus.{FunctionV2S, gradient1, gradientD, resetH}
 ...
 */
class QuasiNewton1 (f: FunctionV2S, g: FunctionV2S = null,
      ...
      extends Minimizer with Error
{
    ...
    {
        var x:  Pair = (x0, gradient1 (fg, x0))        // current (point, gradient)
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** This object is used to test the QuasiNewton1 class.
 */
object QuasiNewton1Test extends App
{
    def x0 = new VectorD (2)

    println ("\nMinimize: (x_0 - 3)^2 + (x_1 - 4)^2 + 1")
    def f (x: VectorD): Double = (x(0) - 3.0) * (x(0) - 3.0) + (x(1) - 4.0) * (x(1) - 4.0) + 1.0
    var solver = new QuasiNewton1 (f)
