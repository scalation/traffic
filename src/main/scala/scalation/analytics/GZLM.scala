...
object GZLM extends GLM
{
     ...
     */
    def apply (x: MatrixD, y: VectorI, cn: Tuple2 [String, String]): LogisticRegression =
    {
        if (add_1)
     ...
     */
    def apply (x: MatrixD, y: VectorI, cn: Array [String]): PoissonRegression =
    {
        if (add_1)
            new PoissonRegression (+: (one (x.dim1), x), y, cn)
        else
            new PoissonRegression (x, y, cn)
    } // apply
