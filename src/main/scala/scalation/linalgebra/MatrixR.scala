...
import scalation.linalgebra.MatrixR.eye
 ...
 */
class MatrixR (val d1: Int,
               val d2: Int,
       private var v:  Array [Array [Rational]] = null)
      extends MatriR with Error with Serializable
{
    // Note: implementations for the following methods are from the MatriR trait:
    ...
    if (v == null) {
        v = Array.ofDim [Rational] (dim1, dim2)
     ...
     */
    def this (u: Array [Array [Rational]]) { this (u.length, u(0).length, u) }
     ...
     */
    def apply (i: Int, j: Int): Rational = v(i)(j)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get this matrix's vector at the i-th index position (i-th row).
     *  @param i  the row index
     */
    def apply (i: Int): VectorR = new VectorR (v(i))
     ...
     */
    def apply (ir: Range, jr: Range): MatrixR = slice (ir.start, ir.end, jr.start, jr.end)
     ...
     *  @param x  the scalar value to assign
     */
    def update (i: Int, j: Int, x: Rational) { v(i)(j) = x }
     ...
     */
    def update (i: Int, u: VectorR) { v(i) = u() }
     ...
     */
    def update (ir: Range, jr: Range, b: MatrixR)
    {
    ...
    {
        val c = new MatrixR (rowIndex.length, dim2)
     ...
     */
    def setCol (col: Int, u: VectorR) { for (i <- range1) v(i)(col) = u(i) }
    ...
    {
        val c = new MatrixR (dim1, colIndex.length)
    ...
    def *= (b: MatrixR): MatrixR =
    {
        ...
        for (i <- range1) {
            val row_i = new VectorR (dim2)            // save ith row so not overwritten
    ...
    def times_ip (b: MatrixR)
    {
        ...
        val bb = if (b == this) new MatrixR (this) else b
        for (i <- range1) {
            val row_i = new VectorR (dim2)            // save ith row so not overwritten
            for (j <- range2) row_i(j) = v(i)(j)      // copy values from ith row of this matrix
            for (j <- range2) {
     ...
     */
    def times_s (b: MatrixR): MatrixR = 
    {
        if (dim2 != b.dim1) flaw ("*", "matrix * matrix - incompatible cross dimensions")

        val c = new MatrixR (dim1, dim1)  // allocate result matrix
    ...
    {
        val l = new MatrixR (dim1, dim2)    // lower triangular matrix
        val u = new MatrixR (this)          // upper triangular matrix (a copy of this)
     ...
     */
    private def partialPivoting (a: MatrixR, i: Int): Int =
    {
    ...
    {
        val b = new MatrixR (this)              // copy this matrix into b
     ...
     */
    def clean (thres: Double, relative: Boolean = true): MatrixR =
    {
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `MatrixR` companion object provides operations for `MatrixR` that don't require
...
object MatrixR extends Error
{
     ...
     *  @param sep       the character separating the values
     */
    def apply (fileName: String, sep: Char = ','): MatrixR =
    {
        ...
        for (i <- 0 until m) x(i) = VectorR (lines(i).split (sep))
        x
    } // apply
     ...
     */
    def eye (n: Int): MatrixR = 
    {
        val c = new MatrixR (n)
        for (i <- 0 until n) c.v(i)(i) = _1
        c
    } // eye
     ...
     */
    def ++ (u: VectorR, a: MatrixR): MatrixR =
    {
        if (u.dim != a.dim2) flaw ("++", "vector does not match row dimension")

        val c = new MatrixR (a.dim1 + 1, a.dim2)
    ...
    def +: (u: VectorR, a: MatrixR): MatrixR =
    {
        ...
        val c = new MatrixR (a.dim1, a.dim2 + 1)
     ...
     */
    def +: (u: VectorR, w: VectorR): MatrixR =
    {
        if (u.dim != w.dim) flaw ("+:", "vector dimensions do not match")

        val c = new MatrixR (u.dim, 2)
        c.setCol (0, u)
        c.setCol (1, w)
        c
    } // +:
     ...
     */
    def outer (x: VectorR, y: VectorR): MatrixR =
    {
        val c = new MatrixR (x.dim, y.dim)
     ...
     *  @param y  the second vector -> row 1
     */
    def form_rw (x: Rational, y: VectorR): MatrixR =
    {
        val cols = y.dim
        val c = new MatrixR (2, cols)
     ...
     */
    def form_rw (x: VectorR, y: Rational): MatrixR =
    {
        val cols = x.dim
        val c = new MatrixR (2, cols)
        c(0) = x
     ...
     *  @param y  the second vector -> column 1
     */
    def form_cw (x: Rational, y: VectorR): MatrixR =
    {
        val rows = y.dim
        val c = new MatrixR (rows, 2)
     ...
     */
    def form_cw (x: VectorR, y: Rational): MatrixR =
    {
        val rows = x.dim
        val c = new MatrixR (rows, 2)
        c.setCol(0, x)
...
object MatrixRTest extends App
{
    ...
    val aa = new MatrixR ((3, 2), 1, 2,
                                  3, 4,
                                  5, 6)
    val bb = new MatrixR ((2, 2), 1, 2,
                                  3, 4)
    ...
    println ("csv = " + MatrixR ("data/matrices/matrix.csv"))

    bb.write ("data/matrices/bb_matrix.csv")
    println ("bb_csv = " + MatrixR ("data/matrices/bb_matrix.csv"))
