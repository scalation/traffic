...
import scalation.linalgebra.MatrixC.eye
...
object MatrixC extends Error
{
     ...
     *  @param sep       the character separating the values
     */
    def apply (fileName: String, sep: Char = ','): MatrixC =
    {
        ...
        for (i <- 0 until m) x(i) = VectorC (lines(i).split (sep))
        x
    } // apply
     ...
     */
    def ++ (u: VectorC, a: MatrixC): MatrixC =
    {
        if (u.dim != a.dim2) flaw ("++", "vector does not match row dimension")

        val c = new MatrixC (a.dim1 + 1, a.dim2)
    ...
    def +: (u: VectorC, a: MatrixC): MatrixC =
    {
        ...
        val c = new MatrixC (a.dim1, a.dim2 + 1)
 ...
 *  > scalation.linalgebra.MatrixCTest
 */
object MatrixCTest extends App
{
    ...
    println ("csv = " + MatrixC ("data/matrices/matrix.csv"))

    bb.write ("data/matrices/bb_matrix.csv")
    println ("bb_csv = " + MatrixC ("data/matrices/bb_matrix.csv"))
