...
import scalation.linalgebra.MatrixL.eye
 ...
 */
class MatrixL (val d1: Int,
               val d2: Int,
       private var v:  Array [Array [Long]] = null)
      extends MatriL with Error with Serializable
{
    // Note: implementations for the following methods are from the MatriL trait:
     ...
     */
    def this (u: Array [Array [Long]]) { this (u.length, u(0).length, u) }
     ...
     */
    def this (dim: Tuple2 [Int, Int], u: Long*)
    {
        this (dim._1, dim._2)
     ...
     */
    def apply (i: Int, j: Int): Long = v(i)(j)

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Get this matrix's vector at the i-th index position (i-th row).
     *  @param i  the row index
     */
    def apply (i: Int): VectorL = new VectorL (v(i))
     ...
     */
    def apply (ir: Range, jr: Range): MatrixL = slice (ir.start, ir.end, jr.start, jr.end)
     ...
     *  @param x  the scalar value to assign
     */
    def update (i: Int, j: Int, x: Long) { v(i)(j) = x }
     ...
     */
    def update (i: Int, u: VectorL) { v(i) = u() }
     ...
     */
    def update (ir: Range, jr: Range, b: MatrixL)
    {
     ...
     */
    def slice (from: Int, end: Int): MatrixL =
    {
        new MatrixL (end - from, dim2, v.slice (from, end))
    } // slice
     ...
     */
    def sliceCol (from: Int, end: Int): MatrixL =
    {
        val c = new MatrixL (dim1, end - from)
     ...
     */
    def slice (r_from: Int, r_end: Int, c_from: Int, c_end: Int): MatrixL = 
    {
        val c = new MatrixL (r_end - r_from, c_end - c_from)
     ...
     */
    def sliceExclude (row: Int, col: Int): MatrixL =
    {
        val c = new MatrixL (dim1 - oneIf (row < dim1), dim2 - oneIf (col < dim2))
     ...
     */
    def selectRows (rowIndex: Array [Int]): MatrixL =
    {
        val c = new MatrixL (rowIndex.length, dim2)
     ...
     */
    def setCol (col: Int, u: VectorL) { for (i <- range1) v(i)(col) = u(i) }
     ...
     */
    def selectCols (colIndex: Array [Int]): MatrixL =
    {
        val c = new MatrixL (dim1, colIndex.length)
    ...
    def *= (b: MatrixL): MatrixL =
    {
        ...
        for (i <- range1) {
            val row_i = new VectorL (dim2)            // save ith row so not overwritten
    ...
    def times_ip (b: MatrixL)
    {
        ...
        val bb = if (b == this) new MatrixL (this) else b
        for (i <- range1) {
            val row_i = new VectorL (dim2)            // save ith row so not overwritten
            for (j <- range2) row_i(j) = v(i)(j)      // copy values from ith row of this matrix
            for (j <- range2) {
     ...
     */
    def times_s (b: MatrixL): MatrixL = 
    {
        if (dim2 != b.dim1) flaw ("*", "matrix * matrix - incompatible cross dimensions")

        val c = new MatrixL (dim1, dim1)  // allocate result matrix
     ...
     *  no partial pivoting.
     */
    def lud_npp: Tuple2 [MatrixL, MatrixL] =
    {
        val l = new MatrixL (dim1, dim2)    // lower triangular matrix
        val u = new MatrixL (this)          // upper triangular matrix (a copy of this)

        for (i <- u.range1) {
            val pivot = u.v(i)(i)
            if (ABS (pivot) < MIN_NORMAL) flaw ("lud_npp", "use lud since you have a zero pivot")
            l.v(i)(i) = 1l
            for (j <- i + 1 until u.dim2) l.v(i)(j) = 0l
     ...
     *  partial pivoting.
     */
    def lud: Tuple2 [MatrixL, MatrixL] =
    {
        val l = new MatrixL (dim1, dim2)         // lower triangular matrix
        val u = new MatrixL (this)               // upper triangular matrix (a copy of this)
     ...
     *  partial pivoting.
     */
    def lud_ip: Tuple2 [MatrixL, MatrixL] =
    {
        val l = new MatrixL (dim1, dim2)         // lower triangular matrix
     ...
     */
    private def partialPivoting (a: MatrixL, i: Int): Int =
    {
     ...
     *  @param b   the constant vector
     */
    def solve (lu: Tuple2 [MatriL, MatriL], b: VectorL): VectorL =
    {
       solve (lu._1, lu._2, b)
    } // solve
     ...
     */
    def diag (p: Int, q: Int): MatrixL =
    {
        if (! isSquare) flaw ("diag", "this matrix must be square")

        val n = dim1 + p + q 
        val c = new MatrixL (n, n)

        for (i <- 0 until p)                       c.v(i)(i) = 1l            // Ip
        for (i <- 0 until dim1; j <- 0 until dim1) c.v(i+p)(j+p) = v(i)(j)    // this
        for (i <- p + dim1 until n)                c.v(i)(i) = 1l            // Iq
        c
    } // diag
     ...
     */
    def setDiag (x: Long) { for (i <- range1) v(i)(i) = x }

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Invert this matrix (requires a square matrix) and does not use partial pivoting.
     */
    def inverse_npp: MatrixL =
    {
        val b = new MatrixL (this)              // copy this matrix into b
     ...
     */
    def clean (thres: Double, relative: Boolean = true): MatrixL =
    {
        val s = if (relative) mag else 1l              // use matrix magnitude or 1
        for (i <- range1; j <- range2) if (ABS (v(i)(j)) <= thres * s) v(i)(j) = 0l
        this
    } // clean
    ...
    {
        for (i <- range1; j <- range2 if v(i)(j) < 0l) return false
        true
    } // isNonegative
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `MatrixL` companion object provides operations for `MatrixL` that don't require
 *  'this' (like static methods in Java). It provides factory methods for building
 *  matrices from files or vectors.
 */
object MatrixL extends Error
{
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
     ...
     */
    def eye (n: Int): MatrixL = 
    {
        val c = new MatrixL (n)
        for (i <- 0 until n) c.v(i)(i) = 1l
        c
    } // eye
     ...
     */
    def ++ (u: VectorL, a: MatrixL): MatrixL =
    {
        if (u.dim != a.dim2) flaw ("++", "vector does not match row dimension")

        val c = new MatrixL (a.dim1 + 1, a.dim2)
    ...
    def +: (u: VectorL, a: MatrixL): MatrixL =
    {
        ...
        val c = new MatrixL (a.dim1, a.dim2 + 1)
     ...
     */
    def +: (u: VectorL, w: VectorL): MatrixL =
    {
        if (u.dim != w.dim) flaw ("+:", "vector dimensions do not match")

        val c = new MatrixL (u.dim, 2)
        c.setCol (0, u)
        c.setCol (1, w)
        c
    } // +:
     ...
     */
    def outer (x: VectorL, y: VectorL): MatrixL =
    {
        val c = new MatrixL (x.dim, y.dim)
     ...
     *  @param y  the second vector -> row 1
     */
    def form_rw (x: Long, y: VectorL): MatrixL =
    {
        val cols = y.dim
        val c = new MatrixL (2, cols)
     ...
     */
    def form_rw (x: VectorL, y: Long): MatrixL =
    {
        val cols = x.dim
        val c = new MatrixL (2, cols)
        c(0) = x
     ...
     *  @param y  the second vector -> column 1
     */
    def form_cw (x: Long, y: VectorL): MatrixL =
    {
        val rows = y.dim
        val c = new MatrixL (rows, 2)
     ...
     */
    def form_cw (x: VectorL, y: Long): MatrixL =
    {
        val rows = x.dim
        val c = new MatrixL (rows, 2)
        c.setCol(0, x)
...
} // MatrixL companion object


//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `MatrixLTest` object tests the operations provided by `MatrixL` class.
 *  > scalation.linalgebra.MatrixLTest
 */
object MatrixLTest extends App
{
    for (l <- 1 to 4) {
        println ("\n\tTest MatrixL on real matrices of dim " + l)
        val x = new MatrixL (l, l)
        val y = new MatrixL (l, l)
        x.set (2)
        y.set (3)
        println ("x + y = " + (x + y))
        println ("x - y = " + (x - y))
    ...
    println ("\n\tTest MatrixL on additional operations")

    val z   = new MatrixL ((2, 2), 1, 2,
                                   3, 2)
    val t   = new MatrixL ((3, 3), 1, 2, 3,
                                   4, 3, 2,
                                   1, 3, 1)
    val b   = VectorL (8, 7)
    val lu  = z.lud
    ...
    val aa = new MatrixL ((3, 2), 1, 2,
                                  3, 4,
                                  5, 6)
    val bb = new MatrixL ((2, 2), 1, 2,
                                  3, 4)
    ...
    println ("csv = " + MatrixL ("data/matrices/matrix.csv"))

    bb.write ("data/matrices/bb_matrix.csv")
    println ("bb_csv = " + MatrixL ("data/matrices/bb_matrix.csv"))
