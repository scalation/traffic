...
import scalation.linalgebra.{MatriD, SymTriMatrixD, VectorD, SparseMatrixD}
import scalation.math.oneIf
import scalation.util.{Error, SortedLinkedHashMap}

//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `ParSparseMatrixD` class stores and operates on Matrices of Doubles.  Rather
 ...
 */
class ParSparseMatrixD (val d1: Int, 
                     val d2: Int)
      extends MatriD with Error with Serializable
{
    // Note: implementations for the following methods are from the MatriD trait:
   ...
   //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
     ...
     */
    def apply (ir: Range, jr: Range): ParSparseMatrixD = slice (ir.start, ir.end, jr.start, jr.end)
     ...
     */
    def update (ir: Range, jr: Range, b: SparseMatrixD)
    {
     ...
     */
    def slice (from: Int, end: Int): ParSparseMatrixD =
    {
        val c = new ParSparseMatrixD (end - from, dim2)
     ...
     */
    def slice (r_from: Int, r_end: Int, c_from: Int, c_end: Int): ParSparseMatrixD =
    {
        val c = new ParSparseMatrixD (r_end - r_from, c_end - c_from)
     ...
     */
    def sliceExclude (row: Int, col: Int): ParSparseMatrixD =
    {
        val c = new ParSparseMatrixD (dim1 - 1, dim2 - 1)
     ...
     */
    def selectRows (rowIndex: Array [Int]): ParSparseMatrixD =
    {
        val c = new ParSparseMatrixD (rowIndex.length, dim2)
     ...
     */
    def selectCols (colIndex: Array [Int]): ParSparseMatrixD =
    {
        val c = new ParSparseMatrixD (dim1, colIndex.length)
     ...
     */
    def t: ParSparseMatrixD =
    {
        val b = new ParSparseMatrixD (dim2, dim1)
     ...
     */
    def ++ (u: VectorD): ParSparseMatrixD =
    {
        if (u.dim != dim2) flaw ("++", "vector does not match row dimension")

        val c = new ParSparseMatrixD (dim1 + 1, dim2)
     ...
     */
    def + (b: ParSparseMatrixD): ParSparseMatrixD =
    {
        val c = new ParSparseMatrixD (this)
     ...
     *  @param b  the matrix to add (requires sameCrossDimensions)
     */
    def + (b: MatrixD): ParSparseMatrixD =
    {
        val c = new ParSparseMatrixD (dim1, dim2)
    ...
    def += (b: ParSparseMatrixD): ParSparseMatrixD =
    {
        for (i <- range1.par; e <- b.v(i)) this(i, e._1) = this(i, e._1) + e._2
        this
    } // +=
    ...
    def += (b: MatrixD): ParSparseMatrixD =
    {
        for (i <- range1.par; j <- range2) this(i, j) = this(i, j) + b(i, j)
        this
    } // +=
     ...
     *  @param x  the scalar to add
     */
    def += (x: Double): ParSparseMatrixD =
    {
        for (i <- range1.par; j <- range2) this(i, j) = this(i, j) + x
        this
    } // +=
    ...
    def * (b: ParSparseMatrixD): ParSparseMatrixD =
    {
        ...
        val c  = new ParSparseMatrixD (dim1, b.dim2)
     ...
     */
    def times_s (b: ParSparseMatrixD): ParSparseMatrixD =
    {
        if (dim2 != b.dim1) flaw ("*", "matrix * matrix - incompatible cross dimensions")

        val c = new ParSparseMatrixD (dim1, dim1)  // allocate result matrix
...
/*
        ...
        if (p > 2) c ~^ (p-1) else c
*/
        var c = new ParSparseMatrixD (this)
     ...
     *  partial pivoting.
     */
    def lud: Tuple2 [ParSparseMatrixD, ParSparseMatrixD] =
    {
        val l = new ParSparseMatrixD (dim1, dim2)   // lower triangular matrix
        val u = new ParSparseMatrixD (this)         // upper triangular matrix (a copy of this)
        for (i <- u.range1) {
            var pivot = u(i, i)
            if (pivot == 0.0) {
     ...
     *  partial pivoting.
     */
    def lud_ip: Tuple2 [ParSparseMatrixD, ParSparseMatrixD] =
    {
        val l = new ParSparseMatrixD (dim1, dim2)   // lower triangular matrix
     ...
     */
    private def partialPivoting (a: ParSparseMatrixD, i: Int): Int =
    {
     ...
     */
    private def swap (a: ParSparseMatrixD, i: Int, k: Int, col: Int)
    {
     ...
     *  @param b   the constant vector
     */
    def solve (lu: Tuple2 [MatriD, MatriD], b: VectorD): VectorD = solve (lu._1, lu._2, b)
     ...
     */
    def diag (b: MatrixD): ParSparseMatrixD =
    {
        val m = dim1 + b.dim1
        val n = dim2 + b.dim2
        val c = new ParSparseMatrixD (m, n)
     ...
     */
    def diag (p: Int, q: Int): ParSparseMatrixD =
    {
        if (! isSymmetric) flaw ("diag", "this matrix must be symmetric")

        val n  = dim1 + p + q
        val c  = new ParSparseMatrixD (n, n)
     ...
     */
    def inverse_npp: ParSparseMatrixD =
    {
        val b = new ParSparseMatrixD (this)           // copy this matrix into b
        val c = new ParSparseMatrixD (dim1, 1.0)      // let c represent the augmentation
        for (i <- b.range1) {
            var pivot = b(i, i)
     ...
     *  partial pivoting.
     */
    def inverse_ip: ParSparseMatrixD =
    {
     ...
     */
    def clean (thres: Double, relative: Boolean = true): ParSparseMatrixD =
    {
...
} // ParSparseMatrixD class


//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `ParSparseMatrixDTest` object is used to test the `ParSparseMatrixD` class.
 */
object ParSparseMatrixDTest extends App
{
    for (l <- 1 to 4) {
        println ("\n\tTest ParSparseMatrixD on real matrices of dim " + l)
        val x = new ParSparseMatrixD (l, l)
        val y = new ParSparseMatrixD (l, l)
        x.set (2.0)
        y.set (3.0)
        println ("x + y  = " + (x + y))
        println ("x - y  = " + (x - y))
    ...
    println ("\n\tTest ParSparseMatrixD on additional operations")

    val z  = new ParSparseMatrixD (2, 2)
    ...
    val w = new ParSparseMatrixD (2, 3)
