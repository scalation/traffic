      ...
      extends MatriI with Error with Serializable
{
     ...
     */
    def clean (thres: Double, relative: Boolean = true): MatrixI =
    {
    ...
    def mean: VectorI =
    {
        var cm = new VectorI (dim2)
        for (j <- range2) cm(j) = col (j).sum / dim1.toInt
        cm
    } // mean
...
object MatrixI extends Error
{
     ...
     */
    def ++ (u: VectorI, a: MatrixI): MatrixI =
    {
        if (u.dim != a.dim2) flaw ("++", "vector does not match row dimension")

        val c = new MatrixI (a.dim1 + 1, a.dim2)
    ...
    def +: (u: VectorI, a: MatrixI): MatrixI =
    {
        ...
        val c = new MatrixI (a.dim1, a.dim2 + 1)
     ...
     */
    def +: (u: VectorI, w: VectorI): MatrixI =
    {
 ...
 *  > scalation.linalgebra.MatrixITest
 */
